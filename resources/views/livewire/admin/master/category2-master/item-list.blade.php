        ...
        <div class="tw:px-[26px]">
            ...
            <div>
                <table class="tw:table-fixed tw:w-[728px] tw:min-w-[728px]">
                    ...
                    <tbody>
                        @forelse ($category2Masters as $category2Master)
                            <tr class="tw:h-[42px] tw:border-b tw:border-b-[#cccccc]">
                                <td class="tw:text-center">{{ $category2Master->id }}</td>
                                <td>{{ $category2Master->item_name }}</td>
                                <td class="tw:text-center">{{ $category2Master->disp_rank }}</td>
                                <td class="tw:h-[41px] tw:flex tw:gap-x-[26px] tw:items-center">
                                    <x-button.blue
                                        class="tw:!h-[21px] tw:!min-w-[72px]"
                                        wire:click="openEditDialog({{ $category2Master->id }})"
                                    >
                                        編集
                                    </x-button.blue>
                                    <div class="tw:flex tw:gap-x-[5px]">
                                        <x-button.gray
                                            ...
                                            wire:click="moveUp({{ $category2Master->id }})"
                                        >
                                            <i class="fas fa-angle-double-up"></i>
                                        </x-button.gray>
                                        <x-button.gray
                                            class="tw:!h-[21px] tw:!min-w-[26px] tw:!px-[0]"
                                            wire:click="moveDown({{ $category2Master->id }})"
                                        >
                                    ...
                                    <x-button.red
                                        ...
                                        x-on:click="if (!confirm('選択した中カテゴリを削除します。よろしいですか？')) { return; } $wire.deleteItem({{ $category2Master->id }});"
                                    >
                                        削除
                                    </x-button.red>
                                </td>
