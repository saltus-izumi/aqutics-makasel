    ...
    $occupantInitialFields = collect($enProgressOccupants)->mapWithKeys(function ($enProgressOccupant) {
        return [
            (string) $enProgressOccupant->id => [
                ...
                'annual_income' => $enProgressOccupant?->annual_income,
                'memo' => $enProgressOccupant?->memo,
            ],
        ];
    })->toArray();
@endphp
    ...
    @foreach ($enProgressOccupants as $enProgressOccupant)
        <div data-occupant-id="{{ $enProgressOccupant->id }}">
            ...
            <div class="tw:flex">
                ...
                <div class="tw:w-[234px] tw:h-[21px] tw:text-center tw:bg-[#f3f3f3] tw:border tw:border-[#cccccc] tw:border-t-0 tw:border-l-0">税込年収</div>
                <div class="tw:w-[338px] tw:h-[21px] tw:text-center tw:bg-[#f3f3f3] tw:border tw:border-[#cccccc] tw:border-t-0 tw:border-l-0">その他</div>
            </div>
            <div class="tw:flex">
                ...
                <div class="tw:w-[234px] tw:h-[42px] tw:text-center tw:border tw:border-[#cccccc] tw:border-t-0 tw:border-l-0">
                    <x-form.input-number name="annual_income" :value="$enProgressOccupant?->annual_income" class="tw:!h-[40px] tw:!text-center" :border="false" />
                </div>
                <div class="tw:w-[338px] tw:h-[42px] tw:text-center tw:border tw:border-[#cccccc] tw:border-t-0 tw:border-l-0">
                    <x-form.input name="memo" :value="$enProgressOccupant?->memo" class="tw:!h-[40px] tw:!text-center" :border="false" />
                </div>
            </div>
        </div>
    @endforeach
...
@push('scripts')
    <script>
        (() => {
            window.enCorporateOccupant = (params = {}) => ({
