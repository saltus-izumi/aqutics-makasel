...
>
        ...
        <div class="tw:flex tw:flex-col tw:justify-center tw:items-center tw:gap-y-[21px]">
            <div class="tw:w-[754px] tw:flex tw:justify-end tw:gap-x-[26px]">
                <x-button.blue
                    ...
                    x-on:click="window.dispatchEvent(new CustomEvent('open-en-corporate-original-contract-control-edit-modal'))"
                >
                    編集
                </x-button.blue>
            </div>
    ...
    <x-modal title="原契約統制｜編集" event="en-corporate-original-contract-control-edit-modal">
        <div class="tw:w-full tw:flex tw:flex-col tw:gap-y-[8px]">
    ...
    <x-modal title="入居審査・与信証跡" event="en-corporate-credit-screening-modal">
        <div class="tw:flex tw:flex-col tw:justify-center tw:items-center tw:gap-y-[21px]">
            <div class="tw:w-[754px] tw:flex tw:justify-end tw:gap-x-[26px]">
                <x-button.blue
                    class="tw:!h-[23px] tw:!px-[15px] tw:!rounded-lg tw:!w-[234px]"
                    x-on:click="window.dispatchEvent(new CustomEvent('open-en-corporate-credit-screening-edit-modal'))"
                >
                    編集
                </x-button.blue>
            </div>
            @foreach ($creditScreeningFileSections as $section)
                <div class="tw:w-[754px] tw:text-[1.3rem] tw:font-bold">
                    {{ $section['title'] }}
                </div>
                @foreach (array_chunk($section['items'], 3, true) as $fileKindRow)
                    <div class="tw:w-[754px] tw:flex tw:gap-x-[26px]">
                        @foreach ($fileKindRow as $fileKind => $fileLabel)
                            @php
                                $fileUrl = $fileUrls[$fileKind] ?? '';
    ...
    <x-modal title="入居審査・与信証跡｜編集" event="en-corporate-credit-screening-edit-modal">
        <div class="tw:w-full tw:flex tw:flex-col tw:gap-y-[8px]">
            @foreach ($creditScreeningFileSections as $section)
                <div class="tw:w-full tw:text-[1.3rem] tw:font-bold">
                    {{ $section['title'] }}
                </div>
                @foreach ($section['items'] as $fileKind => $fileLabel)
                    <div class="tw:flex">
                        <div class="tw:w-[338px] tw:h-[28px] tw:pl-[10px] tw:leading-[27px] tw:bg-[#f3f3f3] tw:border tw:border-[#cccccc]">
                            {{ $fileLabel }}
                        </div>
    ...
    <x-modal title="保証・リスク移転" event="en-corporate-guarantee-risk-transfer-modal">
        <div class="tw:flex tw:flex-col tw:justify-center tw:items-center tw:gap-y-[21px]">
            <div class="tw:w-[754px] tw:flex tw:justify-end tw:gap-x-[26px]">
                <x-button.blue
                    class="tw:!h-[23px] tw:!px-[15px] tw:!rounded-lg tw:!w-[234px]"
                    x-on:click="window.dispatchEvent(new CustomEvent('open-en-corporate-guarantee-risk-transfer-edit-modal'))"
                >
                    編集
                </x-button.blue>
            </div>
            @foreach ($guaranteeRiskTransferFileSections as $section)
                <div>
                    <div class="tw:w-[754px] tw:text-[1.3rem] tw:font-bold">
                        {{ $section['title'] }}
                    </div>
    ...
    <x-modal title="保証・リスク移転｜編集" event="en-corporate-guarantee-risk-transfer-edit-modal">
    ...
    <x-modal title="物件引渡・状態証跡" event="en-corporate-property-handover-status-modal">
        <div class="tw:flex tw:flex-col tw:justify-center tw:items-center tw:gap-y-[21px]">
            <div class="tw:w-[754px] tw:flex tw:justify-end tw:gap-x-[26px]">
                <x-button.blue
                    class="tw:!h-[23px] tw:!px-[15px] tw:!rounded-lg tw:!w-[234px]"
                    x-on:click="window.dispatchEvent(new CustomEvent('open-en-corporate-property-handover-status-edit-modal'))"
                >
                    編集
                </x-button.blue>
            </div>
            @foreach (array_chunk($propertyHandoverFileKinds, 3, true) as $fileKindRow)
                <div class="tw:w-[754px] tw:flex tw:gap-x-[26px]">
                    @foreach ($fileKindRow as $fileKind => $fileLabel)
                        @php
                            $fileUrl = $fileUrls[$fileKind] ?? '';
    ...
    <x-modal title="物件引渡・状態証跡｜編集" event="en-corporate-property-handover-status-edit-modal">
        <div class="tw:w-full tw:flex tw:flex-col tw:gap-y-[8px]">
            @foreach ($propertyHandoverFileKinds as $fileKind => $fileLabel)
                <div class="tw:flex">
                    <div class="tw:w-[338px] tw:h-[28px] tw:pl-[10px] tw:leading-[27px] tw:bg-[#f3f3f3] tw:border tw:border-[#cccccc]">
                        {{ $fileLabel }}
                    </div>
