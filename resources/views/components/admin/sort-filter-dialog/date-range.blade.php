    ...
    <div class="tw:space-y-2">
        ...
        <div>
            ...
            <div class="tw:grid tw:grid-cols-2 tw:gap-2">
                <div>
                    <div class="tw:text-[0.7rem] tw:text-gray-600 tw:mb-1">{{ $fromLabel }}</div>
                    <input
                        type="date"
                        class="tw:w-full tw:border tw:border-gray-300 tw:rounded tw:px-2 tw:py-1 tw:text-sm"
                        placeholder="{{ $placeholderFrom }}"
                        x-model="{{ $filterModel }}.from"
                        x-on:input="handleFilterInput($event)"
                        x-on:keydown.enter.prevent="{{ $onApply }}"
                    >
                </div>
                <div>
                    <div class="tw:text-[0.7rem] tw:text-gray-600 tw:mb-1">{{ $toLabel }}</div>
                    <input
                        type="date"
                        class="tw:w-full tw:border tw:border-gray-300 tw:rounded tw:px-2 tw:py-1 tw:text-sm"
                        placeholder="{{ $placeholderTo }}"
                        x-model="{{ $filterModel }}.to"
                        x-on:input="handleFilterInput($event)"
                        x-on:keydown.enter.prevent="{{ $onApply }}"
                    >
                </div>
